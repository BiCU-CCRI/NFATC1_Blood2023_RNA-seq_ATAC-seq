...
# specify all key information about experiment

# key notes ----
# rlog vs vst - rlog more robust; size factors vary, but will take ages for all the 172 samples
# using blind=FALSE
# shrinkage apeglm
# initial analysis: keeping multiple samples from a patient - patient_oid


# Generating objects for RNA-seq samples 
#project_id="grant_application"
experiment_name = "decon_eda" # 
EXPERIMENT_DIR = "/home/peter_r/deconvolution_project/results/"

# Loading data ----
# loading signatures 
signatures_raw <- readr::read_tsv(file = "deconvolution_project/raw_datasets/metadata/mes_adr_ncc_noradr_genesets.tsv", skip_empty_rows=TRUE) # do not add NA for empty

signatures_list <- list(NCC_like = signatures_raw$`NCC-like`[!is.na(signatures_raw$`NCC-like`)],
...
lapply(signatures_list, length)

# loading metadata and count data
load("data_processed/deconProject_metadata_counts.RData") # "/home/peter_r/data_processed/"

# removing low depth samples
# defined from previous project for Daria
# but check also for other low depth samples!!!!
# potentially remove also very high depth samples?!! - assess effect on predictions!
# [] initially not removing any low depth samples!!!
#low_depth_samples <- c("MNC_018_REL", "MNC_051_DX", "MNC_054_DX", "MNC_066_REL", "DTC_051_DX_2", "DTC_066_REL_3")
low_depth_samples <- ""

# adding factors to 
#Yes – NA stands always for Not Available (also in the OMICS IDs).
#For Disease we have: NB (Neuroblastoma), GN (Ganglioneuroma), GNB (Ganglioneuroblastoma), OS (Osteosarkoma), RMS (Rhabdomyosarcoma), S (Schwanoma)
#For Material we have: DTC (Disseminated Tumor Cells), MNC (Mononuclear cells), TUM (Tumor), BMn (Negative Bone Marrow), BMp (Positive Bone Marrow), CL (Cell line), FB (Fibroblasts), PNF (Peripheral Nerves), SC (Schwann Cells).
#For TP (Timepoint) we have: DE (Diagnosis, TRE (Treatment) and REL (Relapse).

# fix! one sample BM should be BMn! (after discussion with Fikret)
library(dplyr)
coldata <- deconProject_subset_metadata %>%
  dplyr::filter(!(omics_id %in% low_depth_samples)) %>%  # but check also for other low depth samples!!!!
  #dplyr::filter(timepoint == "DE") %>% # only DE
  dplyr::mutate(reads_type = droplevels(reads_type),
                read_length = droplevels(read_length),
                patient_oid = droplevels(patient_oid),
                timepoint = factor(sample_taken_timepoint, levels = c("DX", "REL")),
                disease = as.factor(disease),
                gender = droplevels(gender),
                instrument_platform = droplevels(instrument_platform),
                performed_at = droplevels(performed_at),
                material = factor(material, levels = c("DTC", "MNC", "TUM", "BMn")),
                rnaseq_batch = as.factor(rnaseq_batch),
                library_type = factor(library_type, levels = c("U", "SR"))) %>%
  ...
  dplyr::select(-sample_taken_timepoint) %>%
  dplyr::arrange(material)

count_data <- deconProject_subset_counts%>%
  dplyr::select(all_of(coldata$omics_id))

# doing separate EDA in R notebook

coldata <- coldata %>%
  dplyr::arrange(match(omics_id, names(count_data))) %>%
  as.data.frame()
...
# OUTPUT_DIR = paste0(EXPERIMENT_DIR, "Results/", experiment_name, "/")
OUTPUT_DIR = paste0(EXPERIMENT_DIR, experiment_name, "/")
dir.create(OUTPUT_DIR, showWarnings = TRUE, recursive=TRUE)

FIGURE_DIR = paste0(OUTPUT_DIR,"Figures/")
dir.create(FIGURE_DIR, showWarnings = TRUE, recursive=TRUE)

# Directory to store processed datasets used in the final report generation
OUTPUT_DATASET_DIR = paste0(OUTPUT_DIR,"Datasets/")
#OUTPUT_DATASET_DIR = paste0(EXPERIMENT_DIR, "Datasets/", experiment_name, "/")
dir.create(OUTPUT_DATASET_DIR, showWarnings = TRUE, recursive=TRUE)
...
#library("Biobase")
# library("vsn")
library("biomaRt") # Gene annotation using ensembl database; carafully choose same version that was used for alignment
library(RCurl) # proxy settings for biomaRt  
#options(RCurlOptions = list(proxy="wwwcache.gla.ac.uk:8080",http.version=HTTP_VERSION_1_0)) 

# Data manipulation
library("dplyr")  # data manipulation
library("readr")  # reading files
#library("pryr") # to save/re-run chunks of R code ("macros")

# Plotting
library("ggplot2") # plotting 
library("ggrepel") # repelling text inside plots
library("ggpubr") # publication ready plots; plot matrices
library("factoextra") # PCA plot
library("pheatmap") # heatmaps
library("RColorBrewer")

# install.packages(c("msigdbr", "future", "furrr", "openxlsx"))
library("msigdbr")
library("future")
library("furrr")
...
# [] need clusterProfiler
# [] also use new PCA plots rather than old ones!
...
# generate ensembl annotation for genes in RNA-seq experiment
source(paste0(datasetPreparation_scriptsDIR, "generateEnsemblAnnotation.R"))

# filtering lowly expressed genes either by expression in at least N samples or % of samples
source(paste0(datasetPreparation_scriptsDIR, "filterDatasets.R"))

# selects top N most variable genes selected by either Median Absolute Deviation (mad)
source(paste0(datasetPreparation_scriptsDIR, "rnaSelectTopVarGenes.R"))

# Exploratory data analysis functions ----
EDA_scriptsDIR="/core_bioinformatics/scripts/EDAfunctions/"

# Extract and plot size factor, library depths
# SequencingDepth
# source(paste0(EDA_scriptsDIR, "extractSizeFactors.R"))
# source(paste0(EDA_scriptsDIR, "plotSizeFactors.R"))
#source(paste0(EDA_scriptsDIR, "sequencingDepthCheck.R"))
# normalization
#source(paste0(EDA_scriptsDIR, "normalizationCheck.R"))
# transformation
source(paste0(EDA_scriptsDIR, "transformationCheck.R"))
# cooks distance and pval
source(paste0(EDA_scriptsDIR, "additionalCheck.R"))
# Gene expression plotting
source(paste0(EDA_scriptsDIR, "plotGeneExpression.R"))
# PCA module
source(paste0(EDA_scriptsDIR, "pcaExtractVariance.R"))
source(paste0(EDA_scriptsDIR, "pcaPlotVariance.R"))
source(paste0(EDA_scriptsDIR, "pcaPlotPCs.R"))
source(paste0(EDA_scriptsDIR, "pcaCorrPCs.R"))
source(paste0(EDA_scriptsDIR, "pcaCorrPCsPlot.R"))
source(paste0(EDA_scriptsDIR, "pcaPlotLoadings.R"))
...
source(paste0(FEscripts_scriptsDIR, "convertIDs.R"))
...
# Saving sessionInfo
# package versioning
RunDate = format(Sys.time(), "%d%m%y")
writeLines(capture.output(sessionInfo()), paste0(OUTPUT_DIR, "sessionInfo_", experiment_name, "_", RunDate, ".txt"))
...
if (generateRNAobjects) {
  ...
  if(!file.exists(paste0(OUTPUT_DATASET_DIR, experiment_name, "_dds_log2_vst.RData"))){
    ...
    log2_norm <- normTransform(dds)
    vsd <- vst(dds, blind = TRUE) # blind = TRUE for QC
    #rld <- rlog(dds, blind = TRUE)
    
    # Generate annotation file using biomaRt
    # ensembl = useMart(biomart="ENSEMBL_MART_ENSEMBL", dataset="hsapiens_gene_ensembl", host="http://nov2020.archive.ensembl.org",
    #                   port = 80, verbose =T, version="Ensembl Genes 102")
    # 
    # mart <- useDataset(dataset = "hsapiens_gene_ensembl", ensembl)
    ...
    save(dds, dds_filt, log2_norm, vsd, ensemblAnnot,  file = paste0(OUTPUT_DATASET_DIR, experiment_name, "_dds_log2_vst.RData"))
    
  } else{
    load(paste0(OUTPUT_DATASET_DIR, experiment_name, "_dds_log2_vst.RData"))
  }
}
