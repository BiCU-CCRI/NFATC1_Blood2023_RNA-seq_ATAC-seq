...
import::from(.from = "atacseq_scripts.R", "remove_blacklisted", "add_flag", "load_peaks_to_GRangesList") 
...
import::from(.from = GenomeInfoDb, seqnames)

import::from(.from = ensembldb, seqlevels)
...
if (!file.exists(paste0(results_dir, "/peak_grlist.RDS"))) {
  ...
  peak_gr_raw <- purrr::map(.x = sample_names, .f = function(sample_name) {
    ...
    ChIPseeker::readPeakFile(project_metadata$narrowPeak_files_path[project_metadata$sample_name == sample_name], as = "GRanges")
  ...
  saveRDS(peak_grlist, file = paste0(results_dir, "/peak_grlist.RDS"))
  
} else {
  message("Loading peak_grlist")
  peak_grlist <- readRDS(file = paste0(results_dir, "/peak_grlist.RDS"))
}
...
if (!file.exists(paste0(results_dir, "/QC_results.RData"))){
  ...
  save(peaks_per_chr_plot, atacseqQC, atacseqQC_metrics, atacseqQC_SSDplot, atacseqQC_flags,
       file = paste0(results_dir, "/QC_results.RData"))
} else {
  
  message("Loading QC_results")
  load(file = paste0(results_dir, "/QC_results.RData"))
...
peak_overlaps_df <- tibble::as_tibble(mcols(allPeaksSet_filt_nR)) %>%
  ...
  dplyr::mutate(PID1185 = ifelse(PID1185, NA, paste0("peak",rowid)),
                PID1180 = ifelse(PID1180, NA, paste0("peak",rowid)),
                ND_RJH = ifelse(ND_RJH, NA, paste0("peak",rowid)),
                ND_MK = ifelse(ND_MK, NA, paste0("peak",rowid)))
...
if (!file.exists(paste0(results_dir, "/atacseq_metadata_counts.RData"))){
  ...
  readr::write_tsv(x = counts_consensus_counts_df,
                   file = file.path(results_dir, "atacseq_P_C5_C6_consensus_peaks_counts.tsv"))
...
patient_vs_NDs_results_dir <- file.path(results_dir, "patient_vs_NDs_results")
...
# saving as pdf
pdf(file = file.path(results_dir, paste0("gene_tracks_",gene_toPlot, ".pdf")), width=18, height=10)
kp <- plotKaryotype(zoom = IL2_region, cex=2, genome = "hg38", plot.params = pp)
kpAddBaseNumbers(kp, tick.dist = 10000, minor.tick.dist = 2000,
                add.units = TRUE, cex=2, tick.len = 3)
kpAddMainTitle(kp, paste0("ATAC-seq signal in ", gene_toPlot), cex=2)
...
png(filename = file.path(results_dir, paste0("gene_tracks_",gene_toPlot, ".png")))
kp <- plotKaryotype(zoom = IL2_region, cex=2, genome = "hg38", plot.params = pp)
kpAddBaseNumbers(kp, tick.dist = 10000, minor.tick.dist = 2000,
                add.units = TRUE, cex=2, tick.len = 3)
kpAddMainTitle(kp, paste0("ATAC-seq signal in ", gene_toPlot), cex=2)
