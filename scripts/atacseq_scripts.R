...
#'
#' @examples remove_blacklisted(peak_gr = peak_grlist[[sample_name]]
...
load_peaks_to_GRangesList <- function(peak_files=NULL) {
  ...
  import::here(readPeakFile, .from = ChIPseeker)
  import::here(GRangesList, .from = GenomicRanges)
  
  gr_list <- purrr::map(.x = names(peak_files), .f = function(sample_name) {
...
#' filter_chromosomes_GRangesList
#'
#' @details function to filter out chromosomes from GRangesList
#' 
#' @param granges_list - granges_list
#' @param keep_chromosomes - character vector of chromosomes to keep; default [paste0("chr", c(1:22, "X", "Y"))]
#' @param pruning_mode - see GenomeInfoDb::keepSeqlevels pruning.mode; default ["coarse"]
#' 
#' @importFrom magrittr %>%"
#' @importFrom purrr map
#' @importFrom GenomeInfoDb seqlevelsStyle keepSeqlevels
#' @importFrom GenomicRanges GRangesList
#' 
filter_chromosomes_GRangesList <- function(granges_list=NULL, keep_chromosomes=paste0("chr", c(1:22, "X", "Y")), pruning_mode="coarse") {
  #TODO:
  # [ ] - check granges_list is named
  
  #FIXME:
  # [ ] - fix order of printed chromosomes before and after filtering
  # remove import::here in final package generation!
  import::here("%>%", .from = magrittr)
  import::here(map, .from = purrr)
  import::here(seqlevelsStyle, keepSeqlevels, .from = GenomeInfoDb)
  import::here(GRangesList, .from = GenomicRanges)
  ...
  message("seqlevels style: ", GenomeInfoDb::seqlevelsStyle(granges_list))
  message("chromosomes before filtering:\n", paste(GenomeInfoDb::seqlevels(granges_list), collapse = " "))
  
  gr_list_filt <- purrr::map(.x = names(granges_list), .f = function(sample_name) {
    message("Removing non-standard chr from: ", sample_name)
    GenomeInfoDb::keepSeqlevels(granges_list[[sample_name]],
                                value = keep_chromosomes,
                                pruning.mode = pruning_mode)
  }) %>% setNames(names(granges_list)) %>%
    GenomicRanges::GRangesList(.) # combine into granges list
  
  message("chromosomes after filtering:\n", paste(GenomeInfoDb::seqlevels(gr_list_filt), collapse = " "))
